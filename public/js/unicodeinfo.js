export function init() {
    var _a;
    const w = {};
    (_a = document.getElementById("input")) === null || _a === void 0 ? void 0 : _a.addEventListener("keyup", (e) => {
...
function checktext(w) {
    ...
    const inp = Array.from(inputElement.value);
    // take first n characters
    const trunc = document.getElementById("trunc");
    const take_n = Math.min(inp.length, Number.parseInt(trunc.value));
    clearResult();
    ...
    resultElement === null || resultElement === void 0 ? void 0 : resultElement.appendChild(the_result_tree);
}
function clearResult() {
    document.getElementById("result").innerHTML = "";
}
async function reportChar(w, li, c) {
    var _a;
    let reportText = `<pre class="single-char">${c}</pre>`;
    const codePointStr = (_a = c
        .codePointAt(0)) === null || _a === void 0 ? void 0 : _a.toString(16).toUpperCase().padStart(4, "0");
...
async function loadDataFor(w, searchHeader) {
    // このファイル読むの初めてなので読み出す
    const response = await fetch(`/static/unicode-org/Public/UCD/latest/ucd/UnicodeData/${searchHeader}.txt`);
    if (!response.ok) {
...
class CharInfo {
    constructor(c, codepoint, name, generalCategory, canonicalCombiningClass, bidiClass, decompTypeMap, numericTypeValue0, numericTypeValue1, numericTypeValue2, bidiMirrored, unicodeOneName, isoComment, simpleUppercaseMapping, simpleLowerCaseMapping, simpleTitlecaseMapping) {
        this.c = c;
    ...
    static fromLine(s) {
        ...
        return new CharInfo(c, fields[0], fields[1], fields[2], fields[3], fields[4], fields[5], fields[6], fields[7], fields[8], fields[9], fields[10], fields[11], fields[12], fields[13], fields[14]);
    }
}
